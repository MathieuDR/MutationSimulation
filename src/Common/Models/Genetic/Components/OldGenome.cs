...
public record OldGenome :IBiologicalEncodable {
	...
	public OldGenome(NeuronConnection[] neuronConnections, string? hexSequence = null) {
	...
	public static OldGenome FromBytes(byte[] bytes) => new(GenomesFromBytes(bytes));
	...
	public static OldGenome FromHex(string hex) => new(GenomesFromBytes(Convert.FromHexString(LZString.DecompressFromBase64(hex))), hex);
